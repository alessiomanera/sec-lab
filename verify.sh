...
capture_works() {
    ...
    CAPTURE_ERROR="$(grep -v '^Capturing on' <<< "$CAPTURE_ERROR" | awk 'NF && !shown++' | sed 's/^.*line [0-9]*: //')"
    rm -f "$file"
    return "$rc"
}
...
elif id -nG "$USER_NAME" | grep -w wireshark >/dev/null && ! id -nG | grep -w wireshark >/dev/null; then
